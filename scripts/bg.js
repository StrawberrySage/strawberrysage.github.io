...
function main(){
    ...
    */
    const fsSource = `
        ...
        float bruh(float c1, float c2, float x){
            float fac = 4.;
            if (x < 0.5){
        ...
        float pattern(float c1, float c2, float spd, float offset){

            vec2 uv = gl_FragCoord.xy / vpSize; // 
            float col = 0.;

            float realtimex = spd * time + offset;
        ...
        void main() {
            ...
            vec3 c1 = vec3(0.19607843137, 0., 0.);
            //vec3 c2 = vec3(0.6470588235294118, 0.10980392156862745, 0.18823529411764706);
            vec3 c2 = vec3(0.9, 0., 0.);

            /*vec3 col = vec3(0.);
            for (float v = 1.; v > 0.; v -= 0.2){
              col += mix(c1, c2, pattern(0.2, 0.6, v));
            }*/

            vec3 col = mix(c1, c2, max(pattern(0.2, 0.6, 1., 0.), pattern(0.2, 0.6, 1., 3.)));
            /*const float numv = 10.;
            const float stride = 1.;
            for (float i = 0.; i < numv * stride; i += stride){
              col += mix(c1, c2, pattern(0.2, 0.6, 1., i));
            }
            col /= numv;*/

            /*col += mix(c1, c2, pattern(0.2, 0.6, 1., 0.));
            col += mix(c1, c2, pattern(0.2, 0.6, 1., .5));
            col += mix(c1, c2, pattern(0.2, 0.6, 1., 1.));
            col /= 3.;*/
